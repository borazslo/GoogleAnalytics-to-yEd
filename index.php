<html lang="hu-HU">
	<head>
		<title>Google Analytics to yEd graph</title>
		<style>
			.label {
				float: left;
				height: 10px;
...
<h3>vizsgált rendszer és időszak</h3>
<div class="label">file:</div><input type="text" name="file" value="<?php echo $file; ?>" /><br />
<div class="label">kezdő dátum:</div><input type="text" name="startdate" value="<?php echo $startdate; ?>" /><br />
<div class="label">végső dátum:</div><input type="text" name="enddate" value="<?php echo $enddate; ?>" /><br />
<h3>csomópontok</h3>
<div class="label">csomópontok frissítése:</div><input type="checkbox" name="checknodes" value="1" <?php if($checknodes) echo "checked"; ?> /><br />
<br>
<div class="label">méretezés alapja logaritmikus:</div><input type="radio" name="tipus" value="log" <?php if($tipus == "log") echo "checked"; ?> /> alapja: <input type="text" name="alap" value="<?php echo $alap; ?>" /><br />
<div class="label">méretezés alapja lineáris:</div><input type="radio" name="tipus" value="lin" <?php if($tipus == "lin") echo "checked"; ?> /> meredeksége: <input type="text" name="oszto" value="<?php echo $oszto; ?>" /><br />
<br>
<div class="label">fokszám a méretnél:</div><input type="text" name="fok" value="<?php echo $fok; ?>" /><br />
<div class="label">minimum méret:</div><input type="text" name="forcedmin" value="<?php echo $forcedmin; ?>" /><br />
<div class="label">maximum méret:</div><input type="text" name="forcedmax" value="<?php echo $forcedmax; ?>" /><br />
<h3>vonalak</h3>
<div class="label">vonalak frissítése:</div><input type="checkbox" name="checkedges" value="1" <?php if($checkedges) echo "checked"; ?> /><br />
<br />
<div class="label">látogatás kihagyása:</div><input type="text" name="levon" value="<?php echo $levon; ?>" /><br />
<div class="label">vonalak kihagyása:</div><input type="text" name="kihagy" value="<?php echo $kihagy; ?>" /><br />
...
<?php 
if(!array_key_exists('go',$_GET) AND !array_key_exists('code',$_GET)) exit;


if(file_exists("jsons/ganalytics_".$startdate."-".$enddate.".json")) {
	//echo"!"."ganalytics_".$startdate."-".$enddate.".json";
	db_query("UPDATE domains SET visits = 0;");
	db_query("TRUNCATE ga_source;");
	$json = json_decode(file_get_contents("jsons/ganalytics_".$startdate."-".$enddate.".json"),true);
...
if($checkedges == true) {
	...
	file_put_contents('graphmls/'.$graphml, $source);
